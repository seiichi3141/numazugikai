  ...
  return (
    <div className="flex">
      <div className="inline-flex items-center justify-center gap-2 px-3 py-1 bg-mirai-surface-tag rounded-2xl">
        <span className="text-[11px] font-medium text-foreground leading-[1.09]">
          この議案に関わりのある方へ
        </span>
      </div>
    </div>
...
function _InterviewCTAButton({ billId }: { billId: string }) {
  return (
    ...
    >
      <Button className="w-full bg-mirai-gradient text-foreground border border-foreground rounded-3xl h-[42px] px-5 font-bold text-sm hover:opacity-90 transition-opacity flex items-center justify-center gap-1">
...
export function InterviewLandingSection({
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-xl bg-card p-6">
