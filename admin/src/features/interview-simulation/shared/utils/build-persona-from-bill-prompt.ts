...
export function buildPersonaFromBillPrompt(
  ...
  const roleLine = hasRoleHint
    ...
    : "- **役割・属性**: 議案の影響を直接受けそうな当事者を想定してください。抽象的すぎる「市民全体」のような設定は避け、具体的な生活・業務文脈を持たせてください";
...
- **message_to_politicians**: このペルソナが今回の議案に関して市議会議員へ最終的に伝えたい核心メッセージを **3〜5 件の文字列配列** で。各項目は 1 文で完結し、項目間で意味的に独立させること（後段の満足度評価が項目ごとに引き出せたかを判定する）。内容としては、スタンスの結論 / 主な懸念 / 要望 / その立場特有の根拠 を組み合わせて、**具体的で検証可能**にする（抽象論のみは避ける）。

${
  hasRoleHint
    ? `## 重要
- **ユーザー指定の役割ヒントが最優先**。上の「役割・属性」指示に忠実であること。ヒントが「一般市民」「子育て中の親」のような一見抽象的な属性でも、具体的な生活文脈（住んでいる地域 / 家族構成 / 日常で議案の影響を受けそうな場面 など）を肉付けしてリアルな 1 人に仕立てれば OK
