...
6. **message_to_politicians**: 元レポートの summary / opinions を根拠に、このペルソナが**今回の議案に関して市議会議員へ最終的に伝えたい核心メッセージ**を **3〜5 件の文字列配列** で返してください。各項目は 1 文で、項目間で意味的に独立させること（後段の満足度評価が項目ごとに引き出せたかを判定する）。スタンスの結論 / 主な懸念 / 要望 / その根拠となる立場 を組み合わせ、**具体的で検証可能な内容**にしてください（抽象論だけにしない）。
