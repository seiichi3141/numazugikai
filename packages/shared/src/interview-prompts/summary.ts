...
- 暮らしや地域の具体的な場面が語られている場合は、それが伝わる書き方にする（例:「近所の○○を使っていて」「子どもが通う学校で」）
...
### 3. role（立場・属性）
- ログ内に根拠のある立場のみを用いること（発言に根拠のない立場を推測で付与しない）。複数の立場が読み取れる場合は、その人の意見の説得力・具体性を最もよく説明する立場を優先してよい（例: 議案に関連する職務経験や、対象となる施設・地域との日常的な関わりがあればそれを活かす）
- ただし**過去の経歴と現在の立場は区別する**こと（過去の職歴を現職のように扱わない。詳細は role_title / role_description 参照）
- インタビュイーの立場タイプを以下の4つから**必ず1つ選択すること**:
  - subject_expert: その分野に詳しい有識者・実務経験者
  - work_related: 仕事や業務を通じて関わる
  - daily_life_affected: 暮らしのなかで影響を受ける（対象の施設・地域・サービスを日常的に利用しているなど）
  - general_citizen: 市民として関心がある

### 4. role_description（立場の詳細説明）
- 立場・属性の詳細説明（例：「対象となる施設の近くに20年住み、家族で日常的に利用している」）
...
- 例：「近隣住民」「子育て中の親」「自治会役員」「市内事業者」「介護職」など。発言に根拠のある具体的な立場を端的に表す
- **過去の経歴を現在の職業のように表記しない（過去の職歴なら「元〜」を付ける。例:「元市職員」）**
- **重要**: 必ず10文字以内にすること
