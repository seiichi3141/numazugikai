...
2. push 前に `AGENTS.md` のローカル検証を実行する。
3. 必要に応じて upstream tracking を付けてブランチを `origin` に push する。設定済みのリモート URL をそのまま使う。
4. push がクリーンでない・拒否された場合:
   - 失敗が non-fast-forward や同期問題なら、`pull` スキルを実行して対象 base
     branch をマージし、コンフリクトを解決し、検証を再実行する。
...
7. PR の base が通常開発では対象自治体の develop、hotfix では同じ自治体の main
   と一致することを確認する。
...
# PR の存在を確認（無い場合のみ作成）
pr_state=$(gh pr view --repo seiichi3141/numazugikai \
  --json state -q .state 2>/dev/null || true)
if [ "$pr_state" = "MERGED" ] || [ "$pr_state" = "CLOSED" ]; then
  echo "現在のブランチはクローズ済みの PR に紐づいています。新しいブランチ + PR を作成してください。" >&2
  exit 1
fi

# open PR がなければ branch 名から base を決める。
target_base=$(gh pr view --repo seiichi3141/numazugikai \
  --json baseRefName -q .baseRefName 2>/dev/null || true)
if [ -z "$target_base" ]; then
  target_base=$(node scripts/resolve-branch-base.mjs "$branch")
fi
...
if [ -z "$pr_state" ]; then
  gh pr create --repo seiichi3141/numazugikai \
    --base "$target_base" --title "$pr_title"
else
  # ブランチ更新ごとにタイトルを再考。スコープが変わったら編集する。
  gh pr edit --repo seiichi3141/numazugikai --title "$pr_title"
fi

# PR 本文を .github/pull_request_template.md と一致するように書く／編集する。
# ワークフロー例:
# 1) テンプレートを開きこの PR の本文ドラフトを書く
# 2) gh pr edit --repo seiichi3141/numazugikai --body-file /tmp/pr_body.md
# 3) branch 更新の場合、タイトル／本文が現在の diff と一致するか再確認
# 4) gh pr view --repo seiichi3141/numazugikai --json baseRefName で
#    target_base と一致することを確認

# 返信用の PR URL を表示
gh pr view --repo seiichi3141/numazugikai --json url -q .url
```
