...
- 作成日: 2026-09-03
- 対象: `seiichi3141/shizuokagikai`
- 参照元: `seiichi3141/numazugikai` `develop`
- 状態: 調査・基盤作成段階
...
`numazugikai` の履歴を保持した独立リポジトリとして作成した。

- `origin`: `git@github.com:seiichi3141/shizuokagikai.git`
- `numazu-reference`: 参照元の取得専用。push は無効化する
- 参照元コミット: `4b43ea656d9d8ebcbaa6ffbbf0f950c674df919b`
...
- 独立リポジトリを作成し、参照元への push を無効化
- README と開発ルールを静岡県議会向けへ更新
- 公式データソースと未確定事項を記録
- 参照元から継承した外部デプロイ・DB 操作を、明示的な有効化まで停止
...
- OGP スクリーンショットの定期実行 schedule を、安全確認後に復元
...
外部環境の作成・更新・デプロイは Repository variable
`SHIZUOKA_DEPLOYMENTS_ENABLED` が `true` の場合だけ実行する。この変数は
repository level で管理し、Phase 4 の全検証ゲート通過、静岡県用の Secrets と
environments の設定、実行先の相互確認が完了するまで作成・有効化しない。
課金資源を残さないため、過去に作成済みの preview を削除する cleanup は例外とし、
変数が未設定でも認証情報と対象を確認できた場合だけ実行できるようにする。
